...
#' @docType  class
#' @description Set up a Midae (with bootstrapping where BI=M) imputer object with specified hyperparameters and then obtain multiple imputed datasets
#' @format  NULL
#' @import tensorflow
#' @export
...
Midae.boot2<- R6::R6Class("Midae.boot2",
                     public = list(
                       #input_size
                       n_input =NULL,
                       #latent_size
                       ...
                       train=function(verbose=TRUE,print_freq=1,B=1){
                         tf <- tensorflow::tf
                         tf$compat$v1$disable_v2_behavior()
                         data=self$onehot.df
                         ...
                         for(j in 1:B){
                           ...
                           with(tf$compat$v1$Session(graph=self$vae_graph) %as% sess,{
                             ...
                             dir.path=paste(paste("Temp/MidaebootB",j,sep=""),".ckpt",sep="")
                             self$saver$save(sess,dir.path)
                           })
                       ...
                       impute=function(B=1,I=5,onehot=FALSE,all.numeric=FALSE,add.noise=FALSE,SD=1,pmm=FALSE){
                         tf <- tensorflow::tf
                         data=self$onehot.df
                         #scale data and get colmin and colmax
                         ...
                         for(j in 1:B){
                           ...
                           with(tf$compat$v1$Session(graph=self$vae_graph) %as% sess,{
                             sess$run(tf$compat$v1$global_variables_initializer())
                             restore.path=paste(paste("Temp/MidaebootB",j,sep=""),".ckpt",sep="")
                             ...
                             for(i in 1:I){
                               ...
                               m=I*(j-1)+i
                               onehot.data[[m]]=as.matrix(data)
                               onehot.data[[m]][!notna_loc]<-temp[!notna_loc]
                               onehot.data[[m]]=as.matrix(onehot.data[[m]],ncol=self$n_input)
                               colnames(onehot.data[[m]])=colnames(data)
                             }
